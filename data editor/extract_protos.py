...
def extract(exe_path, output_dir, descriptor_set=None, log=None):
    """Core logic behind the CLI below, also called directly by the web UI's
    Settings tab (see "data editor/routes.py":api_extract_protos). `log`, if
    given, receives one string per line of progress (same lines `main()`
    prints) - the web route collects them into the response instead.

    Returns a summary dict: {n_protos, n_messages, n_fields, outdir, descriptor_set}."""
    def emit(line):
        if log is not None:
            log.append(line)
    ...
    emit("%d .proto files rebuilt -> %s" % (len(seen), outdir))
    emit("%d messages, %d fields" % (total_msg, total_field))

    descriptor_set_written = None
    if descriptor_set:
        fds = descriptor_pb2.FileDescriptorSet()
        for _, fd in sorted(seen.items()):
            fds.file.add().CopyFrom(fd)
        descriptor_set_written = Path(descriptor_set)
        descriptor_set_written.parent.mkdir(parents=True, exist_ok=True)
        descriptor_set_written.write_bytes(fds.SerializeToString())
        emit("FileDescriptorSet -> %s" % descriptor_set_written)

    return {
        "n_protos": len(seen), "n_messages": total_msg, "n_fields": total_field,
        "outdir": str(outdir),
        "descriptor_set": str(descriptor_set_written) if descriptor_set_written else None,
    }
...
def main():
    ...
    log = []
    extract(a.exe, a.output, a.descriptor_set, log=log)
    for line in log:
        print(line)
