...
where the schema no longer matches.
  ...
  types: msg, str, bytes, f32, f64, i32, i64, varint, packed_f32
...
def decode_value(wire, raw, sub_desc=None):
    """-> (type_str, json_value)"""
    if wire == 0:
        return "varint", raw
    if wire == 1:
...
def reserved_numbers(full_name):
    ...
    if _RESERVED is None:
        ...
        try:
            ...
            desc_path = Path(__file__).resolve().parent.parent / "proto" / "acevo.desc"
...
def field_info(desc, number, wire):
    """(name, sub_message_descriptor, enum_descriptor) for a field number.
...
def decode_message(buf, desc=None):
    ...
    for field, wire, raw in iter_fields(buf):
        name, sub, en = field_info(desc, field, wire)
        if name is None and desc is not None and field in reserved_numbers(desc.full_name):
            # marker, not a name: it starts with "?"
            name = "?reserved"
        typ, val = decode_value(wire, raw, sub)
        items.append((field, name, typ, val, enum_name(en, val)))
